...
import type { CreateDocumentRequest, ThesisDocument } from '@/types';
...
export function useImportDocument() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (input: { document: ThesisDocument; templateId?: string | null }) =>
      api.importDocumentJson(input.document, input.templateId ?? null),
    onSuccess: (env) => {
      qc.setQueryData(queryKeys.document(env.id), env);
    }
  });
...
export function useSaveDocument() {
  ...
  return useMutation({
    mutationFn: (input: SaveDocumentInput) =>
      api.saveDocument(input.id, input.document, input.templateId ?? null),
    onSuccess: (env) => {
      qc.setQueryData(queryKeys.document(env.id), env);
    }
  });
...
export function useValidateDocument() {
  return useMutation({
    mutationFn: (input: ValidateInput) => api.validateDocument(input.id, input.templateId ?? null)
  });
}

export function useRenderDocument() {
  return useMutation({
    mutationFn: (input: RenderInput) => api.renderDocument(input.id, input.templateId ?? null)
  });
}
