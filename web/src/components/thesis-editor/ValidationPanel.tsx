...
export function ValidationPanel({ issues, onJump }: { issues: ValidationIssue[]; onJump: (blockId?: string) => void }) {
  if (issues.length === 0) {
    return <EmptyState title="暂无校验问题" description="当前结构可继续导出 JSON。正式 DOCX 仍建议通过后端 validate-input 校验。" />;
  }
  ...
  return (
    <div className="stack" data-testid="validation-panel">
      <div className="issue-summary" aria-label="校验汇总">
        <div className="issue-count"><strong>{counts.error}</strong><span>错误</span></div>
        <div className="issue-count"><strong>{counts.warning}</strong><span>警告</span></div>
        <div className="issue-count"><strong>{counts.info}</strong><span>提示</span></div>
      </div>
      {groups.map(group => {
          ...
          <section className="issue-group" key={group.severity}>
            ...
            {groupIssues.map((issue, index) => (
              <div key={`${issue.code}-${index}`} className={`issue-row ${issue.severity}`}>
                <div className="inline-row issue-row-header">
                  <Badge tone={group.tone}>{issue.severity}</Badge>
                  {issue.blockId ? <Button type="button" onClick={() => onJump(issue.blockId)}>跳转到内容块</Button> : null}
                </div>
                <strong>{issue.message}</strong>
                {issue.path ? <span className="muted">位置：{issue.path}</span> : issue.blockId ? <span className="muted">内容块：{issue.blockId}</span> : null}
