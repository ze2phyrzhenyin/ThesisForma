...
- **作用域**: 旧本地草稿迁移缓存。打开文档时若 `DocumentEnvelope.overrides` 为空，Web 会读取该 key 作为兼容迁移来源。
- **权威存储**: API / local draft envelope 的 `overrides` 字段。它不进入 `ThesisDocument` JSON。

## 5. 后端契约

### 5.1 API

`CreateDocumentRequest` / `SaveDocumentRequest` / `ImportDocumentRequest` / `ValidateDocumentRequest` / `RenderDocumentRequest` 支持可选字段：
...
`schemas/document-overrides.schema.json` 定义该契约。结构与 §2 对齐，所有字段 `additionalProperties: false`。

### 5.2 Renderer 合并点

`DocxRenderContext` 包含 `Overrides` 属性。`DocumentPackageBuilder` / `DocxRenderer` 入口处把模板里的 `ThesisFormatSpec` 和文档的 `Overrides` 合并成 **effective spec**，然后照旧渲染：
...
合并的纯函数实现是 `DocumentOverridesFormatMerger`。渲染测试必须检查 `styles.xml`、`document.xml`、`sectPr`、TOC field、header/footer part，并运行 `OpenXmlValidator`。

### 5.3 验证

- API 层接到 `overrides` 后先做结构/范围校验；不通过返回 400，错误码为 `overrides.validationFailed`。
- `OpenXmlValidator` 不需要改 —— 合并产生的 effectiveSpec 仍然满足现有结构。
- `FormatConformanceValidator` 使用 effectiveSpec 校验渲染结果。
...
| Owner               | 任务                                                                                  |
|---------------------|---------------------------------------------------------------------------------------|
| schema worker       | 维护 `schemas/document-overrides.schema.json` 与 schema tests                         |
| api worker          | 维护 `DocumentEnvelope` 与 request DTO round-trip                                      |
| renderer worker     | 维护 `DocumentOverridesFormatMerger` 与 `sectionInstances` 渲染                         |
| validation worker   | 确保 effectiveSpec 进入 `FormatConformanceValidator`                                  |
| web editor worker   | 从 `OverridesPanel` 写入 envelope；保留 localStorage 迁移窗口                          |
